...
async function main() {
  ...
  const amountDaiToBorrow =
    availableBorrowsETH.toNumber() * 0.95 * (1 / daiPrice.toNumber()) // 95% of availableBorrowsETH
...
async function borrowDai(
  ...
  const borrowTx = await lendingPool.borrow(
    daiAddress, // The address of the token you want to borrow
    amountDaiToBorrow, // amount to borrow
    1, // 1 = Stable rate, 2 = Variable rate
    0, // 0 = No referral
    account // The address that will receive the DAI
  )
